...
import static com.example.teamproject.game02_click.BtnClickGameActivity.currentScore;
...
public class BtnClickGameActivity2 extends AppCompatActivity {

    //SQLITE 데이타베이스 관련변수
    SQLiteDatabase db;
    MySQLiteOpenHelper_click helper;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_02btnclickgame_layout2);

        //데이베이스 생성.
        helper = new MySQLiteOpenHelper_click(
                BtnClickGameActivity2.this, // 현재 화면의 context
                "member2.db", // 파일명
                null, // 커서 팩토리
                1); // 버전 번호
            ...
            switch (v.getId()) {
                case R.id.btnEnter:
                    ...
                    if(et_highscore_name.getText().toString().equals("")) {
                        ...
                        Toast.makeText(BtnClickGameActivity2.this, "이름이 없어 AAA 자동 입력!", Toast.LENGTH_SHORT).show();
                        break;
                    }
...
//                    startActivity(intentJoinOk);
                    ...
                    BtnClickGameActivity PA = (BtnClickGameActivity)BtnClickGameActivity.clickgame; // 다른 액티비티 종료 3, 다른 액티비티에서 객체 생성
                    PA.finish(); // 다른 액티비티 종료 4, 만든 객체로 원하는 작업 실행
                    finish();
                    break;

                case R.id.btnMain:
                    Intent ex1 = new Intent(BtnClickGameActivity2.this, MainActivity.class);
                    startActivity(ex1);
                    finish();
                    mp.stop();
                    break;
                ...
                case R.id.button3:
                    Intent ex2 = new Intent(BtnClickGameActivity2.this, BtnClickGameActivity.class);
                    startActivity(ex2);
                    finish();
                    mp.stop();
                    break;
    ...
    public void onBackPressed() {
        if (BtnClickGameActivity.handler2 != null) {
            BtnClickGameActivity.handler2.removeMessages(0);
            BtnClickGameActivity.handler2.removeCallbacks(null);
            BtnClickGameActivity.handler2.removeCallbacksAndMessages(null);
        }
